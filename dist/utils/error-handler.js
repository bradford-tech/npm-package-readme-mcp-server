...
import { PackageReadmeMcpError, PackageNotFoundError, RateLimitError, NetworkError, } from '../types/index.js';
...
export function handleHttpError(status, response, context) {
    ...
    switch (status) {
        ...
        default:
            throw new PackageReadmeMcpError(`HTTP error ${status}: ${statusText}`, 'HTTP_ERROR', status);
    }
}
export async function withRetry(fn, maxRetries = 3, baseDelay = 1000, context = 'operation') {
    let lastError;
